...
ui.out("something"); ui.blank(); ui.blank(); ui.blank()
check("consecutive blank lines collapse to one",
      buf.getvalue() == "something\n\n", repr(buf.getvalue()))
buf.truncate(0), buf.seek(0)
# A prompt is written without a trailing newline, so the cursor is mid-line and
# the next blank() must actually emit one.
ui.raw("name > "); ui.blank()
check("a blank after an unterminated write still emits",
      buf.getvalue() == "name > \n", repr(buf.getvalue()))
buf.truncate(0), buf.seek(0)
ui.gap()
